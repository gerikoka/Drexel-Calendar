document.addEventListener('DOMContentLoaded', function() {
    ...
    .then(data => {
      if (Array.isArray(data) && data.length > 0) {
        ...
        for (var course of data) {
          ...
          courseItem.dataset.id = course.id; // Set the unique ID
          courseItem.dataset.name = course.name; // Set the course name
  ...
  function deleteCourse(courseData) {
    fetch(`/delete_course/${courseData.id}`, {
      method: 'DELETE',
      headers: {
        'Content-Type': 'application/json',
      },
      ...
      var courseItem = document.querySelector('div[data-id="' + courseData.id + '"]');
      if (courseItem) {
        courseItem.remove();
      }
      ...
      calendar.getEvents().forEach(function(event) {
          if (event.title === courseData.name) {
              event.remove();
